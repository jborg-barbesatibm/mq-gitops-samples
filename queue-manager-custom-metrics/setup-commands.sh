...
oc create secret docker-registry internal-registry-creds --docker-server=image-registry.openshift-image-registry.svc:5000 --docker-username=`oc whoami` --docker-password=`oc whoami -t` --docker-email=barbes@dk.ibm.com
